...
//globals
vector<char*> vjoy::searchPaths;
vector<module*> vjoy::modules;
pthread_mutex_t vjoy::pymutex;

void* module::eventLoop(void* arg) {
	...
	while (true) {
		...
		int s = read(mod->uifd, &evt, sizeof(struct input_event));

		if (s != sizeof(struct input_event)) {
			cerr << "module" << mod->name << ": Error reading event structure." << endl;
			continue;
		}
		...
		case EV_UINPUT:
			...
			case UI_FF_UPLOAD:
				...
				ioctl(mod->uifd, UI_BEGIN_FF_UPLOAD, &ureq);

				pthread_mutex_lock(&vjoy::pymutex);
				pyeffect = vjoy_convert_ff_effect(&ureq.effect);

				res = PyObject_CallMethod(mod->pymodule, const_cast<char*>("doVJoyUploadFeedback"), const_cast<char*>("O"), pyeffect);

				Py_XDECREF(res);
				if (PyErr_Occurred()) PyErr_Print();

				pthread_mutex_unlock(&vjoy::pymutex);
				ioctl(mod->uifd, UI_END_FF_UPLOAD, &ureq);
				break;
			...
			case UI_FF_ERASE:
				...
				ioctl(mod->uifd, UI_BEGIN_FF_ERASE, &ereq);
				pthread_mutex_lock(&vjoy::pymutex);

				res = PyObject_CallMethod(mod->pymodule, const_cast<char*>("doVJoyEraseFeedback"), const_cast<char*>("i"), ereq.effect_id);
				Py_XDECREF(res);
				...
				ioctl(mod->uifd, UI_END_FF_ERASE, &ereq);
				break;
		...
		default:
			...
			res = PyObject_CallMethod(mod->pymodule, const_cast<char*>("doVJoyEvent"), const_cast<char*>("iii"), evt.type, evt.code, evt.value);
			Py_XDECREF(res);
...
void* module::inputLoop(void *arg) {
	module* mod = (module*) arg;
	...
	while (true) {
		...
		pyevents = PyObject_CallMethod(mod->pymodule, const_cast<char*>("doVJoyThink"), NULL);
		...
		if (pyevents) {
			...
			if (eventcount > 0) {
				...
				for (int i = 0; i < eventcount; i++) {
					...
					write(mod->uifd, &evt, sizeof(struct input_event));
				}
...
void vjoy::init() {
	...
	assert(pthread_mutex_init(&vjoy::pymutex, NULL) == 0);
	...
	path << Py_GetPath() << ":" << getenv("HOME") << "/.config/vjoy/modules:" << cwd << ":/usr/lib/share/pyshared";
