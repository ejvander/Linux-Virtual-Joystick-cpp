...
//as well as input coming from the python script, possibly relayed from external hardware
void* device::inputLoop(void* arg) {
	...
	while (true) {
		...
		if (dev->enableFF) {
			...
			if (s == -1) {
				if (errno != EAGAIN) {
					cerr << "could not read device file; " << strerror(errno) << endl;
		...
		if (pyevents) {
			...
			if (eventcount > 0) {
				...
				for (int i = 0; i < eventcount; i++) {
					...
					int wrote = write(dev->uifd, &evt, sizeof(input_event));
					if (wrote != sizeof(input_event)) {
						if (wrote == -1) cerr << strerror(errno) << endl;
						else cerr << "could not write event update, wrote " << wrote << endl;
					}
				}
...
void vjoy::init(const char* extraPath) {
	...
	path << Py_GetPath() << ":" << getenv("HOME") << ":.:" << cwd << ":" << extraPath;
	cout << endl;
	...
	if (extraPath) PyList_Append(pyPath, PyString_FromString(extraPath));
